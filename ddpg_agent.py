...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done, epsilon_decay, noise_reset):
        ...
        if self.t_step == 0 and len(self.memory) > BATCH_SIZE:
            for _ in range(NUM_LEARNING):
                ...
                self.learn(experiences, GAMMA, epsilon_decay, noise_reset)
        
    def act(self, state, random_noise):
        """Returns actions for given state as per current policy."""
        state = torch.from_numpy(state).float().to(device)
        
        # switch mode, eval() just evaluate, train() can be trained
        ...
        if random_noise:
            action += self.epsilon*np.random.uniform(-0.2,0.2,np.shape(action))
        else:
    ...
    def learn(self, experiences, gamma, epsilon_decay, noise_reset):
